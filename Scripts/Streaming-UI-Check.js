...
event-interaction https://raw.githubusercontent.com/chinming17/Quantumult-X-Icons/main/Scripts/Streaming-UI-Check.js, tag=串流媒體-解鎖查詢, img-url=checkmark.seal.system, enabled=true
...
const policy_name = "Netflix" //填入你的 Netflix 策略組名
...
let result = {
  "title": '    📺  串流媒體服務查詢',
  "YouTube": '<b>YouTube: </b>檢測失敗，請重試 ❗️',
  "Netflix": '<b>Netflix: </b>檢測失敗，請重試 ❗️',
  "Dazn": "<b>Dazn: </b>檢測失敗，請重試 ❗️",
  "Disney": "<b>Disney+: </b>檢測失敗，請重試 ❗️",
  "Paramount" : "<b>Paramount+: </b>檢測失敗，請重試 ❗️",
  "Discovery" : "<b>Discovery+: </b>檢測失敗，請重試 ❗️",
  "ChatGPT" : "<b>ChatGPT: </b>檢測失敗，請重試 ❗️"
  //"Google": "Google 定位: 檢測失敗，請重試"
...
;(async () => {
  ...
  if (status==STATUS_COMING) {
    //console.log(1)
    result["Disney"] = "<b>Disney+:</b> 即將登陸 ➟ "+'⟦'+flags.get(region.toUpperCase())+"⟧ ⚠️"
  } else if (status==STATUS_AVAILABLE){
    //console.log(2)
    result["Disney"] = "<b>Disney+:</b> 支援 ➟ "+'⟦'+flags.get(region.toUpperCase())+"⟧ 🎉"
    console.log(result["Disney"])
  } else if (status==STATUS_NOT_AVAILABLE) {
    //console.log(3)
    result["Disney"] = "<b>Disney+:</b> 未支援 🚫 "
  } else if (status==STATUS_TIMEOUT) {
    result["Disney"] = "<b>Disney+:</b> 檢測超時 🚦 "
  }
  ...
  content = content + "</br>------------------------------</br>"+"<font color=#CD5C5C >"+"<b>節點</b> ➟ " + $environment.params+ "</font>"
...
$configuration.sendMessage(message).then(resolve => {
    ...
    if (resolve.ret) {
      ...
      content = content + "</br>--------------------------------------</br>"+"<font color=#CD5C5C>"+"<b>節點</b> ➟ " + output+ "</font>"
      content =`<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + content + `</p>`
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response => {
      ...
      if (response.statusCode === 404) {
        ...
        result["Netflix"] = "<b>Netflix: </b>支援自製劇集 ⚠️"
        ...
        //console.log("nfnf")
        result["Netflix"] = "<b>Netflix: </b>未支援 🚫"
        console.log("nf:"+result["Netflix"])
        ...
        result["Netflix"] = "<b>Netflix: </b>完整支援"+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
        //$notify("nf:"+result["Netflix"])
      ...
      result["Netflix"] = "<b>Netflix: </b>檢測超時 🚦"
...
function testYTB() { 
    ...
    $task.fetch(option).then(response=> {
      ...
      if (response.statusCode !== 200) {
        //reject('Error')
        result["YouTube"] = "<b>YouTube Premium: </b>檢測失敗 ❗️"
      } else if (data.indexOf('Premium is not available in your country') !== -1) {
          //resolve('Not Available')
        result["YouTube"] = "<b>YouTube Premium: </b>未支援 🚫"
      ...
      //resolve(region)
      result["YouTube"] = "<b>YouTube Premium: </b>支援 "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
      console.log("ytb:"+region+ result["YouTube"])
      }
    }, reason => {
      result["YouTube"] = "<b>YouTube Premium: </b>檢測超時 🚦"
      //resolve("timeout")
    })
}
...
function testDazn() { 
  ...
  $task.fetch(option).then(response=> {
    ...
    if (response.statusCode !== 200) {
      ...
      if (ret != null && ret.length === 2) {
        region = ret[1]
        result["Dazn"] = "<b>Dazn: </b>支援 "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
      } else {
        result["Dazn"] = "<b>Dazn: </b>未支援 🚫"
...
function testParam() { 
  ...
  $task.fetch(option).then(response=> {
    ...
    if (response.statusCode == 200) {
      //reject('Error')
      result["Paramount"] = "<b>Paramount+: </b>支援 🎉 "
    } else if (response.statusCode == 302) {
      //resolve('Not Available')
      result["Paramount"] = "<b>Paramount+: </b>未支援 🚫"
    } 
      console.log("Paramount+:"+ result["Paramount"])
  }, reason => {
    result["Paramount"] = "<b>Paramount+: </b>檢測超時 🚦"
    //resolve("timeout")
  })
}
...
function testDiscovery() {
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response=> {
      ...
      const cookievalid =`_gcl_au=1.1.858579665.1632206782; _rdt_uuid=1632206782474.6a9ad4f2-8ef7-4a49-9d60-e071bce45e88; _scid=d154b864-8b7e-4f46-90e0-8b56cff67d05; _pin_unauth=dWlkPU1qWTRNR1ZoTlRBdE1tSXdNaTAwTW1Nd0xUbGxORFV0WWpZMU0yVXdPV1l6WldFeQ; _sctr=1|1632153600000; aam_fw=aam=9354365;aam=9040990; aam_uuid=24382050115125439381416006538140778858; st=${token}; gi_ls=0; _uetvid=a25161a01aa711ec92d47775379d5e4d; AMCV_BC501253513148ED0A490D45@AdobeOrg=-1124106680|MCIDTS|18894|MCMID|24223296309793747161435877577673078228|MCAAMLH-1633011393|9|MCAAMB-1633011393|RKhpRz8krg2tLO6pguXWp5olkAcUniQYPHaMWWgdJ3xzPWQmdj0y|MCOPTOUT-1632413793s|NONE|vVersion|5.2.0; ass=19ef15da-95d6-4b1d-8fa2-e9e099c9cc38.1632408400.1632406594`
      let option1 = {
      ...
      $task.fetch(option1).then(response=> {
        ...
        if (locationd == "us") {
          result["Discovery"] = "<b>Discovery+: </b>支援 🎉 "
          console.log("支援Discovery+")
          resolve("支援Discovery+")
          return
        } else {
          result["Discovery"] = "<b>Discovery+: </b>未支援 🚫"
          console.log("不支援Discovery+")
          resolve("不支援Discovery+")
          return
        }
      }, reason => {
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response=> {
      ...
      $task.fetch(option1).then(response=> {
        ...
        if (res != -1) {
          result["ChatGPT"] = "<b>ChatGPT: </b>支援 "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
          console.log("支援 ChatGPT")
          resolve("支援 ChatGPT")
          return
        } else {
          result["ChatGPT"] = "<b>ChatGPT: </b>未支援 🚫"
          console.log("不支援 ChatGPT")
          resolve("不支援 ChatGPT")
          return
        }
      }, reason => {
